...
export function LessonEditForm({ lesson, onUpdated, onCancel }: LessonEditFormProps) {
  const {
    register,
    handleSubmit,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="border-border space-y-3 rounded-lg border p-3">
      <div className="space-y-1">
        <Label htmlFor={`edit-lesson-title-${lesson.id}`} className="text-xs">タイトル</Label>
        <Input
      ...
      <div className="space-y-1">
        <Label htmlFor={`edit-notion-id-${lesson.id}`} className="text-xs">Notion ページ URL / ID</Label>
        <Input
          id={`edit-notion-id-${lesson.id}`}
          className="h-7 text-sm"
      ...
      <div className="flex items-center gap-4">
        <div className="space-y-1">
          <Label htmlFor={`edit-lesson-order-${lesson.id}`} className="text-xs">順序</Label>
          <Input
